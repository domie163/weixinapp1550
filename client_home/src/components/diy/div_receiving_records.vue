<template>
	<view class="diy_details diy_div_receiving_records">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','whether_to_identify')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>是否鉴定:</span>
						</view>
							<view class="diy_field diy_text">
							<text>
								{{obj["whether_to_identify"]}}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','appraisal_fees')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>鉴定费用:</span>
						</view>
							<view class="diy_field diy_text">
					...
					<view v-if="$check_field('get','pickup_method')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>取货方式:</span>
						</view>
							<view class="diy_field diy_text">
					...
					<view v-if="$check_field('get','consignee')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>收货人:</span>
						</view>
							<view class="diy_field diy_text">
					...
					<view v-if="$check_field('get','receiving_phone_number')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>收货电话:</span>
						</view>
							<view class="diy_field diy_text">
					...
					<view v-if="$check_field('get','receiving_time')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>收货时间:</span>
						</view>
							<view class="diy_field diy_date">
							<text>
								{{ $toTime(obj["receiving_time"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','receiving_remarks')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>收货备注:</span>
						</view>
							<view class="diy_field diy_desc">
							<text>
								{{ obj["receiving_remarks"] }}
							</text>
						</view>
						</view>
				</view>
			...
			<view class="container">
				<view class="row">
					<view class="col">
						<button type="button" class="diy_btn" @click="to_form('/pages/after_sales_application/edit')" v-if="$check_action('/after_sales_application/edit','add') || $check_action('/after_sales_application/edit','set')" >售后申请</button>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/receiving_records/get_obj", {
					receiving_records_id:this.query.receiving_records_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["receiving_records_id"] !==0) {
