<template>
	<view class="diy_details diy_div_after_sales_application">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','after_sales_quantity')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>售后数量:</span>
						</view>
					...
					<view v-if="$check_field('get','after_sales_issues')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>售后问题:</span>
						</view>
							<view class="diy_field diy_text">
			...
			<view class="container">
				<view class="row">
					<view class="col">
						<button type="button" class="diy_btn" @click="to_form('/pages/after_sales_results/edit')" v-if="$check_action('/after_sales_results/edit','add') || $check_action('/after_sales_results/edit','set')" >售后处理</button>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/after_sales_application/get_obj", {
					after_sales_application_id:this.query.after_sales_application_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["after_sales_application_id"] !==0) {
...
<style scoped>
	.div_after_sales_application_details {}
