<template>
	<view class="diy_details diy_div_reply_record">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','reply_content')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>回复内容:</span>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/reply_record/get_obj", {
					reply_record_id:this.query.reply_record_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["reply_record_id"] !==0) {
