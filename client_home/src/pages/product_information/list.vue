			...
			<view class="container">
					...
					<view>
						<view class="">
							<!-- 搜索栏 -->
											...
											@input="input($event, 'trade_name')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																												<uni-search-bar placeholder="搜索供货商名" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'supplier_name')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
														<!-- /搜索栏 -->
						</view>
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
																		<uni-section title="商品类型" type="line">
							<uni-data-select
									@change="change_product_type"
		...
		<!-- 列表 -->
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
							<view class="col" v-if="1 && $check_field('get','commodity_map')">
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>商品图</span>
							</view>
										<view class="diy_field diy_img">
								<img :src="$fullUrl(o['commodity_map'])" width="100%" height="100" />
							</view>
									</view>
					</view>
						...
						<view class="view">
							...
							<view class="col" v-if="1 && $check_field('get','stock')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>库存</span>
							</view>
						...
						<view class="view">
							...
							<view class="col" v-if="0 && $check_field('get','product_introduction')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>商品介绍</span>
							</view>
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?product_information_id=' + o['product_information_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
		...
		data() {
			return {
				url_get_list: "~/api/product_information/get_list?like=0",
									// 商品类型选项列表
				list_product_type: [{value:"全部",text:"全部"}],
											// 库存选项列表
				list_stock: [{value:"全部",text:"全部"}],
											// 用户列表
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=product_information", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["product_information_id"] === o["source_id"]){
