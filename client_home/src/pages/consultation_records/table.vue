<template>
  <view class="page_consultation_records diy_table diy_table_tml" id="consultation_records_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                        <uni-forms-item label="咨询主题" name="consultation_topic">
                      <uni-easyinput type="text" v-model="query.consultation_topic" placeholder="咨询主题" />
                    </uni-forms-item>
                  </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/consultation_records/view?" v-show="user_group == '管理员' || $check_action('/consultation_records/table','add') || $check_action('/consultation_records/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','consultation_topic')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>咨询主题</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','consultation_content')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>咨询内容</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/consultation_records/table','del') || $check_action('/consultation_records/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/consultation_records/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/consultation_records/table','set') || $check_action('/consultation_records/view','set') || $check_action('/consultation_records/view','get')" class="btn btn-primary">详情</navigator>
					                    																		<!--跨表按钮-->
															<a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/reply_record/view','add')" @click="to_form('/pages/reply_record/view',o)">
									<span>回复</span>
								</a>
																						</view>
			</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/consultation_records/get_list?like=0",
				url_del: "~/api/consultation_records/del?",
