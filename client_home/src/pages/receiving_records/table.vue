<template>
  <view class="page_receiving_records diy_table diy_table_tml" id="receiving_records_table">
    <!-- 筛选模块(开始) -->
    <view>
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="订单号" name="order_number">
                    ...
                    <uni-forms-item label="商品名称" name="trade_name">
                      ...
                      <uni-easyinput type="text" v-model="query.pickup_method" placeholder="取货方式" />
                    </uni-forms-item>
                          </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/receiving_records/view?" v-show="user_group == '管理员' || $check_action('/receiving_records/table','add') || $check_action('/receiving_records/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','order_quantity')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>订购数量</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','receiving_time')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>收货时间</span>
							</view>
								...
								<span>
									{{ $toTime(o["receiving_time"] ,"yyyy-MM-dd") }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','receiving_remarks')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>收货备注</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/receiving_records/table','del') || $check_action('/receiving_records/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/receiving_records/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/receiving_records/table','set') || $check_action('/receiving_records/view','set') || $check_action('/receiving_records/view','get')" class="btn btn-primary">详情</navigator>
					                    																		<!--跨表按钮-->
															<a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/after_sales_application/view','add')" @click="to_form('/pages/after_sales_application/view',o)">
									<span>售后申请</span>
								</a>
																						</view>
			</view>
