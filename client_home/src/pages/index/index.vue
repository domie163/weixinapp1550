<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="商品信息推荐"
      url="/pages/product_information/list"
      :list="list_article"
      v-if="$check_action('/product_information/list', 'get')"
    >
	  <list_product_information :list="list_product_information"></list_product_information>
		...
		*/
		get_product_information() {
			...
			this.$get(url, {
					...
					console.log("商品信息" ,json.result.list);
					this.list_product_information = json.result.list;
											this.list_product_information.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_product_information ,"product_information" ,"34670");
									}
			});
		},
