<template>
	<view class="page_appointment_order diy_list diy_list_page div_list_tml" id="appointment_order_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
																																	<uni-section title="取货方式" type="line">
							<uni-data-select
									@change="change_pickup_method"
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?appointment_order_id=' + o['appointment_order_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
			...
			return {
				url_get_list: "~/api/appointment_order/get_list?like=0",
					...
					list_user_supplier_user: [],
																// 是否鉴定选项列表
				list_whether_to_identify: [{value:"全部",text:"全部"}],
							// 鉴定费用选项列表
				list_appraisal_fees: [{value:"全部",text:"全部"}],
											// 用户列表
					list_user_user_name: [],
								// 取货方式选项列表
				list_pickup_method: [{value:"全部",text:"全部"}],
	...
	get_user_user_name(id){
		...
		return ret;
							...
							// 改变分类标签
	change_pickup_method(val) {
		if (val === "全部" || val === "") {
			this.query.pickup_method = "";
		}else{
			this.query.pickup_method = val
		}
		this.search();
	},
