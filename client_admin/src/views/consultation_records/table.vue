<template>
	<el-main class="bg table_wrap">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/consultation_records/table','add') || $check_action('/consultation_records/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/consultation_records/table','del') || $check_action('/consultation_records/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/consultation_records/table','set') || $check_action('/consultation_records/view','set') || $check_action('/consultation_records/view','get') || $check_action('/回复/table','add') || $check_action('/回复/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/consultation_records/table','set') || $check_action('/consultation_records/view','set') || $check_action('/consultation_records/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
						<!--跨表按钮-->
							<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/reply_record/view')" v-if="user_group == '管理员' || $check_action('/reply_record/table','add') || $check_action('/reply_record/view','add')">
						<span>回复</span>
					</el-button>
							</template>
			</el-table-column>
