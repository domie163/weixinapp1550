			...
			<el-row>
					...
					<el-form-item label="商品名称">
									<el-input v-model="query.trade_name"></el-input>
																																	...
																																	<el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap">
					<el-form-item label="售后问题">
									<el-input v-model="query.after_sales_issues"></el-input>
								</el-form-item>
				</el-col>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/after_sales_application/table','add') || $check_action('/after_sales_application/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/after_sales_application/table','del') || $check_action('/after_sales_application/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','supplier_name')" min-width="200">
					...
					<el-table-column prop="after_sales_quantity" @sort-change="$sortChange" label="售后数量"
				v-if="user_group == '管理员' || $check_field('get','after_sales_quantity')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','shipping_address')" min-width="200">
					...
					<el-table-column prop="after_sales_issues" @sort-change="$sortChange" label="售后问题"
				v-if="user_group == '管理员' || $check_field('get','after_sales_issues')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/after_sales_application/table','set') || $check_action('/after_sales_application/view','set') || $check_action('/after_sales_application/view','get') || $check_action('/售后处理/table','add') || $check_action('/售后处理/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/after_sales_application/table','set') || $check_action('/after_sales_application/view','set') || $check_action('/after_sales_application/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
						<!--跨表按钮-->
							<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/after_sales_results/view')" v-if="user_group == '管理员' || $check_action('/after_sales_results/table','add') || $check_action('/after_sales_results/view','add')">
						<span>售后处理</span>
					</el-button>
							</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/after_sales_application/get_list?like=0",
				url_del: "~/api/after_sales_application/del?",
