		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item label="取货方式">
									<el-input v-model="query.pickup_method"></el-input>
								</el-form-item>
				</el-col>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/receiving_records/table','add') || $check_action('/receiving_records/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/receiving_records/table','del') || $check_action('/receiving_records/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','shipping_address')" min-width="200">
					...
					<el-table-column prop="receiving_time" @sort-change="$sortChange" label="收货时间"
				v-if="user_group == '管理员' || $check_field('get','receiving_time')" min-width="200">
		                <template slot-scope="scope">
                	{{ $toTime(scope.row["receiving_time"],"yyyy-MM-dd") }}
                </template>
					</el-table-column>
					<el-table-column prop="receiving_remarks" @sort-change="$sortChange" label="收货备注"
				v-if="user_group == '管理员' || $check_field('get','receiving_remarks')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/receiving_records/table','set') || $check_action('/receiving_records/view','set') || $check_action('/receiving_records/view','get') || $check_action('/售后申请/table','add') || $check_action('/售后申请/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/receiving_records/table','set') || $check_action('/receiving_records/view','set') || $check_action('/receiving_records/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
						<!--跨表按钮-->
							<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/after_sales_application/view')" v-if="user_group == '管理员' || $check_action('/after_sales_application/table','add') || $check_action('/after_sales_application/view','add')">
						<span>售后申请</span>
					</el-button>
							</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/receiving_records/get_list?like=0",
				url_del: "~/api/receiving_records/del?",
