	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="订单号" prop="order_number">
							  ...
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','order_number')) || (!form['after_sales_results_id'] && $check_field('add','order_number'))" :disabled="disabledObj['order_number_isDisabled']"></el-input>
				...
				<el-form-item label="商品名称" prop="trade_name">
							  ...
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','trade_name')) || (!form['after_sales_results_id'] && $check_field('add','trade_name'))" :disabled="disabledObj['trade_name_isDisabled']"></el-input>
				...
				<el-form-item label="商品类型" prop="product_type">
							  ...
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','product_type')) || (!form['after_sales_results_id'] && $check_field('add','product_type'))" :disabled="disabledObj['product_type_isDisabled']"></el-input>
				...
				<el-form-item label="供货商用户" prop="supplier_user">
													<el-select v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','supplier_user')) || (!form['after_sales_results_id'] && $check_field('add','supplier_user'))" id="supplier_user" v-model="form['supplier_user']" :disabled="disabledObj['supplier_user_isDisabled']">
				...
				<el-form-item label="供货商名" prop="supplier_name">
							  ...
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','supplier_name')) || (!form['after_sales_results_id'] && $check_field('add','supplier_name'))" :disabled="disabledObj['supplier_name_isDisabled']"></el-input>
				...
				<el-form-item label="售后数量" prop="after_sales_quantity">
												<el-input id="after_sales_quantity" v-model="form['after_sales_quantity']" placeholder="请输入售后数量"
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','after_sales_quantity')) || (!form['after_sales_results_id'] && $check_field('add','after_sales_quantity'))" :disabled="disabledObj['after_sales_quantity_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','after_sales_quantity')">{{form['after_sales_quantity']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','user_name') || $check_field('add','user_name') || $check_field('set','user_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="用户名称" prop="user_name">
													<el-select v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','user_name')) || (!form['after_sales_results_id'] && $check_field('add','user_name'))" id="user_name" v-model="form['user_name']" :disabled="disabledObj['user_name_isDisabled']">
				...
				<el-form-item label="收货人" prop="consignee">
							  ...
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','consignee')) || (!form['after_sales_results_id'] && $check_field('add','consignee'))" :disabled="disabledObj['consignee_isDisabled']"></el-input>
				...
				<el-form-item label="收货电话" prop="receiving_phone_number">
							  ...
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','receiving_phone_number')) || (!form['after_sales_results_id'] && $check_field('add','receiving_phone_number'))" :disabled="disabledObj['receiving_phone_number_isDisabled']"></el-input>
				...
				<el-form-item label="收货地址" prop="shipping_address">
							  ...
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','shipping_address')) || (!form['after_sales_results_id'] && $check_field('add','shipping_address'))" :disabled="disabledObj['shipping_address_isDisabled']"></el-input>
				...
				<el-form-item label="售后问题" prop="after_sales_issues">
							  ...
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','after_sales_issues')) || (!form['after_sales_results_id'] && $check_field('add','after_sales_issues'))" :disabled="disabledObj['after_sales_issues_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','after_sales_issues')">{{form['after_sales_issues']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','after_sales_processing') || $check_field('add','after_sales_processing') || $check_field('set','after_sales_processing')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="售后处理" prop="after_sales_processing">
												<el-input id="after_sales_processing" v-model="form['after_sales_processing']" placeholder="请输入售后处理"
							  v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','after_sales_processing')) || (!form['after_sales_results_id'] && $check_field('add','after_sales_processing'))" :disabled="disabledObj['after_sales_processing_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','after_sales_processing')">{{form['after_sales_processing']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','after_sales_remarks') || $check_field('add','after_sales_remarks') || $check_field('set','after_sales_remarks')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="售后备注" prop="after_sales_remarks">
								<el-input type="textarea" id="after_sales_remarks" v-model="form['after_sales_remarks']" placeholder="请输入售后备注"
						v-if="user_group === '管理员' || (form['after_sales_results_id'] && $check_field('set','after_sales_remarks')) || (!form['after_sales_results_id'] && $check_field('add','after_sales_remarks'))" :disabled="disabledObj['after_sales_remarks_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','after_sales_remarks')">{{form['after_sales_remarks']}}</div>
							</el-form-item>
			</el-col>
